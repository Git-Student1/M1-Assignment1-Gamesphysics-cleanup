 ...
 */
import { BALL_STOP_WIDTH, PLANK_WIDTH, Seesaw } from "./seesaw.js";
...
import {
	collisionBallsAir,
	checkCollisionWithSegments,
	collisionBalls,
	collisionTimeBalls2D,
	calculateCollisionBallsOnSegment,
} from "./collisionChecking.js";
...
//seesawBallStop
let seesawBallStopX =
	seesawBaseX + PLANK_WIDTH / 2 - ballDiameter - BALL_STOP_WIDTH / 2;

export let seesawLeft = new SeesawLeft(
	-seesawBaseX,
	bottom,
	maxRotation,
	seeSawSupportHeight,
...
export let seesawRight = new SeesawRight(
	seesawBaseX,
	bottom,
	maxRotation,
	seeSawSupportHeight,
...
//throwing ball
let throwBallLeftX =
	seesawLeft._ballStopX - seesawLeft._ballStopWidth - throwBallDiameter / 4;
let throwBallRightX =
	seesawRight._ballStopX + seesawRight._ballStopWidth + throwBallDiameter / 4;
let throwBallYStart =
	seesawLeft._plankCenterY +
	seesawLeft._plankHeight / 2 +
	throwBallDiameter / 2;

let throwballLeft = new ThrowBall(
	throwBallLeftX,
	throwBallYStart,
	throwBallDiameter,
	seesawLeft._seesawBaseX,
	seesawLeft._plankCenterY,
	throwballColor,
	throwballMass
);
let throwballRight = new ThrowBall(
	throwBallRightX,
	throwBallYStart,
	throwBallDiameter,
	seesawRight._seesawBaseX,
	seesawRight._plankCenterY,
	throwballColor,
	throwballMass
);
...
let sensitiveCircleLeft = new SensitiveCircle(
	sensitiveCircleLeftXStart,
	sensitiveCircleYStart,
	throwBallDiameter,
	seesawBaseLeftX,
	seesawPlankCenterY
);
let sensitiveCircleRight = new SensitiveCircle(
	sensitiveCircleRightXStart,
	sensitiveCircleYStart,
	throwBallDiameter,
	seesawBaseRightX,
	seesawPlankCenterY
...
function setup() {
	...
	let seesawLeftPoints = calulateEndPointsOfLine(
		seesawLeft.plankTopCenter,
		-maxRotation,
		seesawLeft._plankWidth
	);
	let seesawRightPoints = calulateEndPointsOfLine(
		seesawRight.plankTopCenter,
		maxRotation,
		seesawRight._plankWidth
	);
	...
	segmentBallDataCochonnet = new SegmentBallData(
		segmentArray[1],
		segmentArray[1].length / 2
	);
	...
	sensitiveCircleLeft.rotateAroundSeesawCenter(
		radians(seesawLeft.rotation._rotationValue)
	);
	//update values of throw ball
	throwballLeft.rotateAroundSeesawCenter(
		radians(seesawLeft.rotation._rotationValue)
	);

	//updateValues of the sensitive circle
	sensitiveCircleRight.rotateAroundSeesawCenter(
		radians(seesawRight.rotation._rotationValue)
	);
	//update values of throw ball
	throwballRight.rotateAroundSeesawCenter(
		radians(seesawRight.rotation._rotationValue)
	);
}
...
function calulateEndPointsOfLine(centerPoint, rotatationDegree, length) {
	if (!centerPoint.x || !centerPoint.y)
		throw new Error("point parameter has no x and/or y value");
...
function createSegmentArray(segmentPointArray) {
	...
	for (let i = 0; i < segmentPointArray.length - 1; i++) {
		...
		segmentArray.push(
			new Segment(segmentPointArray[i], segmentPointArray[i + 1])
		);
	}
	return segmentArray;
}
...
function addLimiter(previousSegment, currentSegment, throwBallDiameter) {
	if (!(previousSegment instanceof Segment))
		throw new Error("segmentArray is not an instance of Segment");
	if (!(currentSegment instanceof Segment))
		throw new Error("segmentArray is not an instance of Segment");

	let degree = Vector.randians(
		previousSegment.segmentVector,
		currentSegment.segmentVector
	);
...
function setAdjacentSegment(previousSegment, currentSegment) {
	if (!(previousSegment instanceof Segment))
		throw new Error("previousSegment is not an instance of Segment");
	if (!(currentSegment instanceof Segment))
		throw new Error("currentSegment is not an instance of Segment");
...
function addSegmentProperties(segmentArray, throwBallDiameter) {
	if (!(segmentArray instanceof Array))
		throw new Error("segmentArray is not an instance of Array");
	for (let i = 0; i < segmentArray.length; i++) {
		if (i > 0) {
			...
			if (!(previousSegment instanceof Segment))
				throw new Error(
					"segmentArray at index " +
						i +
						" is not an instance of Segment"
				);
			if (!(currentSegment instanceof Segment))
				throw new Error(
					"segmentArray at index " +
...
function draw() {
	...
	if (gameData.playerTurn === gameData.playerTurns.PLAYER1)
		gameProcess(
			...
			throwballRight,
			stateRight,
			segmentBallDataRight,
			cochonnet,
			segmentBallDataCochonnet
		);
	else if (gameData.playerTurn === gameData.playerTurns.PLAYER2)
		gameProcess(
			seesawRight,
			throwballRight,
			sensitiveCircleRight,
			stateRight,
			segmentBallDataRight,
			wind,
			throwballLeft,
	...
	fill(255, 0, 0);
	rect(
		kXi(redBound1CenterX * M),
		kYi(redBoundStartY * M),
		redBoundWidth * M,
		redBoundHeight * M
	);
	rect(
		kXi(redBound2CenterX * M),
		kYi(redBoundStartY * M),
		redBoundWidth * M,
		redBoundHeight * M
	);
...
function showResult(cochonnet) {
	if (!(cochonnet instanceof ThrowBall))
		throw new Error("cochonnet parameter  is not of type ThrowBall");
...
function gameProcess(
	...
	if (!(seesaw instanceof Seesaw))
		throw new Error("seesaw parameter  is not of type Seesaw");
	if (!(throwball instanceof ThrowBall))
		throw new Error("throwball parameter  is not of type ThrowBall");
	if (!(sensitiveCircle instanceof SensitiveCircle))
		throw new Error(
			"sensitiveCircle parameter  is not of type SensitiveCircle"
		);
	if (!(state instanceof State))
		throw new Error("state parameter is not of type State");
	if (!(wind instanceof Wind))
		throw new Error("wind parameter is not of type Wind");
	if (!(otherThrowBall instanceof ThrowBall))
		throw new Error("otherThrowBall parameter  is not of type ThrowBall");
	if (!(otherThrowBallState instanceof State))
		throw new Error("otherThrowBallState parameter is not of type State");
	if (!(cochonnet instanceof ThrowBall))
		throw new Error("cochonnet parameter  is not of type ThrowBall");
	if (!(segmentBallData instanceof SegmentBallData))
		throw new Error(
			"segmentBallData parameter  is not of type SegmentBallData"
		);
	if (!(otherSegmentBallData instanceof SegmentBallData))
		throw new Error(
			"otherSegmentBallData parameter  is not of type SegmentBallData"
		);
	...
	switch (state.state) {
		case state.ONSEESAW: {
			//mouse seesaw controll
			{
				...
				if (seesaw._currentlyDragged) {
					seesaw.rotation.calulateRotationValueOfSeesaw(
						M,
						mouseX,
						mouseY
					);
			...
			if (seesaw._throw) {
				let velocity =
					seesaw.calculateThrowBallStartVelocityAfterSpringRotation(
						throwball.centerX,
						throwball.centerY
					);
			...
			//updateValues of the sensitive circle
			sensitiveCircle.rotateAroundSeesawCenter(
				radians(seesaw.rotation.rotationValue)
			);
			//update values of throw ball
			throwball.rotateAroundSeesawCenter(
				radians(seesaw.rotation.rotationValue)
			);
			break;
		}
		...
		case state.ONFLIGHT: {
			...
			let collisionAllowed =
				gameData.playerTurn === gameData.playerTurns.PLAYER1
					? !collisionAirLeft
					: !collisionAirRight;
			if (
				collisionBallsAir(throwball, cochonnet, dt) &&
				collisionAllowed
			) {
				console.log("collision Air");
				let timeUntilColllision = collisionTimeBalls2D(
					throwball,
					cochonnet
				);
				throwball.calulateNextPositionObliqueThrow(
					timeUntilColllision,
					wind.speed
				);
				cochonnet.calulateNextPositionOnSlope(
					timeUntilColllision,
					segmentBallDataCochonnet
				);
				collisionBallsAir(cochonnet, throwball);
				throwball.calulateNextPositionObliqueThrow(
					dt - timeUntilColllision,
					wind.speed
				);
				cochonnet.calulateNextPositionOnSlope(
					dt - timeUntilColllision,
				...
				if (gameData.playerTurn === gameData.playerTurns.PLAYER1)
					collisionAirLeft = true;
				if (gameData.playerTurn === gameData.playerTurns.PLAYER2)
					collisionAirRight = true;
			} else {
				throwball.calulateNextPositionObliqueThrow(
					dt,
					wind.speed * 0,
			...
			if (otherThrowBallState.state === otherThrowBallState.ONSLOPE) {
				otherThrowBall.calulateNextPositionOnSlope(
					dt,
					otherSegmentBallData
				);
			}
		...
		case state.ONSLOPE: {
			...
			deltaTimeOverLimitThrowball = handleMovementOverSegmentLimits(
				throwball,
				segmentBallData
			);
			deltaTimeOverLimitCochonnet = handleMovementOverSegmentLimits(
				cochonnet,
				cochonnetSegmentBallData
			);
			if (otherThrowBallState.state === otherThrowBallState.ONSLOPE)
				deltaTimeOverLimitOtherThrowball =
					handleMovementOverSegmentLimits(
						otherThrowBall,
						otherSegmentBallData
					);

			if (
				throwball.isCoastDown() &&
				otherThrowBall.isCoastDown() &&
				cochonnet.isCoastDown()
			) {
				gameData.nextTurn();
				...
				switch (otherThrowBallState.state) {
					case otherThrowBallState.ONSLOPE: {
						if (collisionBalls(throwball, otherThrowBall, dt)) {
							...
							collisionBalls(otherThrowBall, cochonnet, dt)
						) {
							console.log("collision");
							let timeUntilCollision = collisionCase(
								otherThrowBall,
			...
			throwball.calulateNextPositionOnSlope(
				dt - deltaTimeOverLimitThrowball - timeUntilCollisionThrowball,
				segmentBallData
			);
			cochonnet.calulateNextPositionOnSlope(
				dt - deltaTimeOverLimitCochonnet - timeUntilCollisionCochonnet,
				cochonnetSegmentBallData
			);
			switch (otherThrowBallState.state) {
				case otherThrowBallState.ONSLOPE:
...
function handleMovementOverSegmentLimits(throwball, segmentBallData) {
	...
	if (segmentBallData.overLeftLimit()) {
		let deltaS =
			segmentBallData._segment.getLimiterLeft() -
			segmentBallData._pointOnSegment;
		deltaTimeOverLimit = Math.abs(
			deltaS / throwball.getOnSlopeVelocity(segmentBallData)
		);
		segmentBallData = segmentBallData.dataForStartOnLeftSegment();
	} else if (segmentBallData.overRightLimit()) {
		let deltaS =
			segmentBallData._pointOnSegment -
			segmentBallData._segment.getLimiterRight();
		deltaTimeOverLimit = Math.abs(
			deltaS / throwball.getOnSlopeVelocity(segmentBallData)
		);
...
function collisionCase(ball1, ball2) {
