...
export const PLANK_HEIGHT = 0.006;
export const PLANK_WIDTH = 0.25;
export const BALL_STOP_WIDTH = 0.02;
export const BALL_STOP_HEIGHT = 0.01;
export class Seesaw {
	/**
	...
	constructor(
		...
		this._pullDegreeBelowStraight = 0.0;
		this._throw = false;
		this._seesawPlankMaxRotationForSec = 2500;
		this._velocity = 0;
		...
		this._plankHeight = PLANK_HEIGHT;
		this._plankWidth = PLANK_WIDTH;
		this._ballStopWidth = BALL_STOP_WIDTH;
		this._ballStopHeight = BALL_STOP_HEIGHT;

		this._plankCenterY =
			this._seesawBaseY + this._supportHeight - this._plankHeight / 2;
		this._ballStopBottomY = this._plankCenterY + this._plankHeight / 2;

		this._calulateSupportData();
	}
	get plankTopCenter() {
	...
	_calulateSupportData() {
		this._supportEdgeLength =
			this._supportHeight / Math.sin((60 / 180) * Math.PI);
	}
	/**
	...
	draw(M, rotationValue) {
		fill(0, 0, 255);
		this._drawSupport(M);
		this._drawRotatables(M, rotationValue);
	}
	...
	activateRotationIfDragged() {
		if (this._currentlyDragged) {
			...
			if (this.rotation.rotateUpCondition()) {
				...
				this.rotation._pullDegreeBelowStraight = 0;
			}
		}
		this._currentlyDragged = false;
	}
	...
	rotateUpIfCondition(dt) {
		if (this._rotateUp) {
			this.rotation.rotationValue -=
				this.calulateRotationDifferenceForDrag(
					this._pullDegreeBelowStraight,
					this._maxRotation,
					dt
				);
	...
	applySpringRotation(dt, ballMass) {
		...
		let sRestPosition =
			rotationRadiusAtBallPosition * radians(this.rotation.upperBorder);
		let sCurrently =
			rotationRadiusAtBallPosition * radians(this.rotation.rotationValue);
		let deltaS = sRestPosition - sCurrently;

		//calulate acceleration and velocity
		let acceleration =
			gravitation -
			2 * damping * this._velocity -
			selfOscillationSq * deltaS;
		...
		let rotationDegreeDiv = degrees(
			(this._velocity / rotationRadiusAtBallPosition) * dt
		);
	 ...
	 * @returns
	 */
	calulateRotationDifferenceForDrag(
		seesawPullDegreeBelowStraight,
		maxRotation,
		dt
	) {
	...
	calulateRotataionSpeed() {
		...
		let rotatationSpeed =
			rotationPercent * this._seesawPlankMaxRotationForSec;
	...
	calculateThrowBallStartVelocityAfterRotation(throwballX, throwballY) {
		let throwBallDistanceToRotationCenter = this.calulateDistance(
			this._seesawBaseX,
			this._plankCenterY,
			throwballX,
			throwballY
		...
		let velocity =
			(2 * Math.PI * throwBallDistanceToRotationCenter) / secondsPerRound;
		//calulate the movement direction of the things on the seesaw
		let ballMovementDirectionDegree =
			this.rotation.get90DegreeAngleToCurrentOne();
		if (ballMovementDirectionDegree < -180)
			this.rotation.rotationValue += 360;

		let velocityX =
			-velocity * Math.cos(radians(ballMovementDirectionDegree));
		let velocityY =
			velocity * Math.sin(radians(ballMovementDirectionDegree));
		return { x: velocityX, y: velocityY };
	}

	calculateThrowBallStartVelocityAfterSpringRotation() {
	...
	_drawSupport(M) {
		this._drawTriangle(
			this._seesawBaseX * M,
			this._seesawBaseY * M,
			this._supportEdgeLength * M,
			this._supportHeight * M
		);
	}
	_drawRotatables(M, rotationValue) {
		push();
		rectMode(CENTER);
		...
		rect(
			kXi(this._seesawBaseX * M),
			kYi(this._plankCenterY * M),
			this._plankWidth * M,
			this._plankHeight * M
		);
		this._drawTriangle(
			this._ballStopX * M,
			this._ballStopBottomY * M,
			this._ballStopWidth * M,
			this._ballStopHeight * M
		);
		pop();
	}
	...
	_drawTriangle(
		triangleCenterX,
		bottom,
		triangleBottomEdgeLength,
		triangleHeight
	) {
		triangle(
			kXi(triangleCenterX - triangleBottomEdgeLength / 2),
			kYi(bottom),
			kXi(triangleCenterX),
			kYi(bottom + triangleHeight),
			kXi(triangleCenterX + triangleBottomEdgeLength / 2),
			kYi(bottom)
		);
	}
...
export default class SeesawLeft extends Seesaw {
	...
	constructor(
		...
		this.rotation = new SeesawRotationLeft(
			seesawBaseX,
			this._plankCenterY,
			maxRotation
		);
...
export class SeesawRight extends Seesaw {
	...
	constructor(
		...
		super(seesawBaseX, seesawBaseY, maxRotation, supportHeight, ballStopX);
		this.rotation = new SeesawRotationRight(
			seesawBaseX,
			this._plankCenterY,
			maxRotation
		);
