...
export default class SegmentBallData {
	...
	getCurrentSegmentBallData() {
		if (this._pointOnSegment < this._segment._limitLeft) {
			return new SegmentBallData(
				this._segment.getSegmentLeft(),
				this._segment.getSegmentLeft().getLimiterRight()
			);
		}
		if (this._pointOnSegment > this._segment._limitRight) {
			return new SegmentBallData(
				this._segment.getSegmentRight(),
				this._segment.getSegmentRight().getLimiterLeft()
			);
		}
		return this;
	}
	...
	copyFromData(segmentBallData) {
		if (!(segmentBallData instanceof SegmentBallData))
			throw new Error(
				"segmentBallData is not instance of SegmentBallData"
			);
 ...
 */
export function checkCollisionWithSegments(
	ball,
	segmentArray,
	segmentBallData
) {
	if (!(ball instanceof ThrowBall))
		throw new Error("ball is not instance of Throwball");
	if (!(segmentArray instanceof Array))
		throw new Error("segmentArray is not instance of Array");
	if (!(segmentBallData instanceof SegmentBallData))
		throw new Error("segmentBallData is not instance of SegmentBallData");
...
export function checkCollisionWithSegent(ball, segment) {
	if (!(segment instanceof Segment))
		throw new Error("segment is not instance of Segment");
...
export function checkCollisionBallLine(ball, linePoint1, linePoint2) {
	...
	if (typeof linePoint1.x !== "number" || typeof linePoint1.y !== "number")
		throw new Error("linePoint1 parameter has no x and/or y value");
	if (typeof linePoint2.x !== "number" || typeof linePoint2.y !== "number")
		throw new Error("linePoint2 parameter has no x and/or y value");
	...
	let crossProduct =
		lineVector.x * linePointToPointVector.y -
		lineVector.y * linePointToPointVector.x;
	let lineVectorLength = Math.sqrt(
		Math.pow(lineVector.x, 2) + Math.pow(lineVector.y, 2)
	);
	...
	if (
		ball.centerX + ball.diameter / 2 < minX ||
		ball.centerX - ball.diameter / 2 > maxX
	)
		return false;
...
function calulateCollisionPoint(ball, segment) {
	...
	let distance =
		Vector.cross(segmentEndPointToBallVector, segment.segmentVector) /
		segment.segmentVector.length();
	...
	let segmentPartPosition =
		Vector.dot(segmentEndPointToBallVector, segment.segmentVector) /
		segment.segmentVector.length();

	if (segmentPartPosition < 0 || -distance - ball.diameter * 0.5 > 0)
		return new SegmentBallData(null, null);

	return new SegmentBallData(segment, segmentPartPosition);
}

export function collisionBalls(ball1, ball2, dt) {
	if (!(ball1 instanceof ThrowBall))
		throw new Error("ball1 is not instance of Throwball");
	if (!(ball2 instanceof ThrowBall))
		throw new Error("ball2 is not instance of Throwball");
...
export function collisionTimeBalls2D(ball1, ball2) {
	let collisionTimes = [];
	let collisionTime1 =
		(ball1.diameter / 2 +
			ball2.diameter / 2 +
			Math.sqrt(
				Math.pow(ball1.centerX - ball2.centerX, 2) +
					Math.pow(ball2.centerY - ball1.centerY, 2)
			)) /
		Math.sqrt(
			Math.pow(ball2.velocityX - ball1.velocityX, 2) +
				Math.pow(ball2.velocityY - ball1.velocityY, 2)
		);
	let collisionTime3 =
		(ball1.diameter / 2 +
			ball2.diameter / 2 +
	...
	let collisionTime5 =
		(-(ball1.diameter / 2 + ball2.diameter / 2) +
			Math.sqrt(
				Math.pow(ball1.centerX - ball2.centerX, 2) +
					Math.pow(ball2.centerY - ball1.centerY, 2)
			)) /
	...
	collisionTimes.push(
		...[
			collisionTime1,
			/*collisionTime2,*/ collisionTime3,
			/*collisionTime4,*/ collisionTime5,
			collisionTime6,
			collisionTime7,
			collisionTime8,
		]
	...
	collisionTimes.forEach((collisionTime) => {
		if (collisionTime < smallestTime && collisionTime > 0) {
...
export function collisionTimeBalls1D(ball1, ball2, logTimes) {
	...
	if (collisionTime1 < collisionTime2 && collisionTime1 > 0)
		return collisionTime1;
...
export function calculateCollisionBallsOnSegment(ball1, ball2) {
	...
	if (!(ball2 instanceof ThrowBall))
		throw new Error("ball2 is not instance of Throwball");
	//elastic collision
	let newVelocityXBall1 =
		((ball1.mass - ball2.mass) * ball1.velocityX +
			2 * ball2.mass * ball2.velocityX) /
		(ball1.mass + ball2.mass);
	let newVelocityXBall2 =
		((ball2.mass - ball1.mass) * ball2.velocityX +
			2 * ball1.mass * ball1.velocityX) /
		(ball1.mass + ball2.mass);
...
export function calulateCollisionBallsInAir(ballOnSegment, ballInAir) {
	if (!(ballOnSegment instanceof ThrowBall))
		throw new Error("ballOnSegment is not instance of Throwball");
	if (!(ballInAir instanceof ThrowBall))
		throw new Error("ballInAir is not instance of Throwball");
