import { iXk, iYk, seesawLeft, seesawRight } from "./Game_Uebung_11";
...
export default class SeesawRotation {
	 ...
	 */
	constructor(rotationCenterX, rotationCenterY, rotationOffset) {
		//in degree
	 ...
	 */
	limitRotation() {
		if (this.outsideRotationBorderLower())
			this._rotationValue = this.lowerBorder;
		if (this.outsideRotationBorderUpper())
	...
	calulateRotationValueOfSeesaw(M, mouseX, mouseY) {
		let deltaMouseAndRotationCenterX =
			this.rotationCenterX * M - iXk(mouseX);
		let deltaMouseAndRotationCenterY =
			this.rotationCenterY * M - iYk(mouseY);
		// console.log("X: " + deltaMouseAndRotationCenterX)
		// console.log("Y: " + deltaMouseAndRotationCenterY)

		let rotationValueUncorrected = degrees(
			Math.atan2(
				deltaMouseAndRotationCenterY,
				deltaMouseAndRotationCenterX
			)
		);
		let rotationValueCorrected = this.correctRotationValueForMouseDrag(
			rotationValueUncorrected
		);
...
export class SeesawRotationLeft extends SeesawRotation {
	...
	outsideRotationBorderLower() {
		...
		if (
			this._rotationValue < this.lowerBorder &&
			this._rotationValue > -180 + this.rotationOffset
		)
			return true;
		return false;
	}
...
export class SeesawRotationRight extends SeesawRotation {
	...
	outsideRotationBorderLower() {
		if (this._rotationValue > this.lowerBorder && this._rotationValue < 180)
			return true;
		return false;
	}
	outsideRotationBorderUpper() {
	 ...
	 */
	correctRotationValueForMouseDrag(rotationValueUncorrected) {
		if (rotationValueUncorrected < 0)
			return -180 - rotationValueUncorrected;
